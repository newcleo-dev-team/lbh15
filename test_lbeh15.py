...
class LeadTester(unittest.TestCase):
    ...
    def test_p_s(self):
        for leadP in leadPs:
            ...
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromPs.T_in_celsius, tol)

    def test_sigma(self):
        for leadP in leadPs:
            fromSigma = lead.LeadSigma(leadP.sigma)
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromSigma.T_in_celsius, tol)

    def test_rho(self):
        for leadP in leadPs:
            fromRho = lead.LeadRho(leadP.rho)
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromRho.T_in_celsius, tol)

    def test_alpha(self):
        for leadP in leadPs:
            fromAlpha = lead.LeadAlpha(leadP.alpha)
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromAlpha.T_in_celsius, tol)

    def test_u_s(self):
        for leadP in leadPs:
            fromU_s = lead.LeadU_s(leadP.u_s)
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromU_s.T_in_celsius, tol)

    def test_beta_s(self):
        for leadP in leadPs:
            fromBeta_s = lead.LeadBeta_s(leadP.beta_s)
            self.assertAlmostEqual(leadP.T_in_celsius,
                                   fromBeta_s.T_in_celsius, tol)
...
class BismuthTester(unittest.TestCase):
    ...
    def test_p_s(self):
        for bismuthP in bismuthPs:
            ...
            self.assertAlmostEqual(bismuthP.T_in_celsius,
                                   fromPs.T_in_celsius, tol)
...
class LBETester(unittest.TestCase):
    ...
    def test_p_s(self):
        for lbeP in lbePs:
            ...
            self.assertAlmostEqual(lbeP.T_in_celsius,
                                   fromPs.T_in_celsius, tol)
